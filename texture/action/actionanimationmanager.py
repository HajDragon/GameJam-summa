...
class ActionAnimationManager(object):
    ...
    def loadActionTextureFiles(self, actionType :ActionType) -> Animation:
        ...
        filename = TextureHelper.get_resource_path("data/textures/action/{}.ascii".format(actionName))
        animation = TextureHelper.readAnimationFile(filename)
        animation.name = actionName

        filenameYaml = TextureHelper.get_resource_path("data/textures/action/{}.yaml".format(actionName))
